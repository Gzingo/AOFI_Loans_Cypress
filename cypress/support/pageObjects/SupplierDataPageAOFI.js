...
export class SupplierDataPageAOFI {
  ...
  clickFinishAndGoToNextButton() {
    ...
    cy.get('app-supplier-data').within(() => {
  ...
  deleteAllFirstMatchingDomesticSupplier() {
    const name = getTestData.domesticSupplierNameAOFI();
    const emptyPlaceholders = getTestData.emptyPlaceholdersAOFI();
    cy.intercept('POST', '/api/aofiRequest/deleteDomesticOrForeign').as('deleteDomesticSupplier');
    function deleteFirstRow() {
      cy.get('app-domestic-foreign-grid .e-gridcontent tbody tr')
        .first()
        .then(($row) => {
          ...
          cy.closeAllPopups();
          cy.wait('@deleteDomesticSupplier');
          // recursive
          deleteFirstRow();
        });
    }
    deleteFirstRow();
    cy.contains('app-domestic-foreign-grid .e-gridcontent tbody tr', name, {
      timeout: 2000,
    }).should('not.exist');
