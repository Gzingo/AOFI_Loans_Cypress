    ...
    setupNodeEvents(on, config) {
      ...
      on('task', {
        ...
        stopSmtp() {
          ...
          try {
            execSync('taskkill /IM Rnwood.Smtp4dev.exe /F', { stdio: 'ignore' });
            console.log('SMTP server zaustavljen');
          } catch (e) {
            console.log('SMTP server nije bio pokrenut');
          }
          return true;
        },
        waitForSmtpReady() {
        ...
        async sendTestEmail(to) {
          try {
            ...
            const baseUrl = process.env.CYPRESS_TUBEIQ_URL;
            if (!baseUrl) throw new Error('CYPRESS_TUBEIQ_URL environment variable is required');
            const confirmationUrl = `${baseUrl.replace(/\/$/, '')}/Account/ConfirmEmail?userId=${userId}&code=${code}&spaceId=0&plan=Professional`;
            const htmlBody = `
